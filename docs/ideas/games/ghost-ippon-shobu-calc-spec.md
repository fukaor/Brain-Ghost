...
> **変更履歴**:
> - v1.4 (2026-05-17): 速度カーブを 3 段階離散 (warmup/normal/push) → start_ms / end_ms の**線形補間**に変更。「途中から急に早くなる」UX 問題を解消。同時に T1〜T3 (slow クラス) のベースラインを 600→300ms から 800→500ms に緩和し、初心者の参入障壁を下げる。
...
| ティア | 数字枚数 | 桁数 | 表示間隔（最遅 → 最速） | 答えの最大桁 | スコア倍率 |
|---|---|---|---|---|---|
| **T1** | 10 枚 | 1 桁 | 0.80 → 0.50 秒 | 2 桁 | × 1.0 |
| **T2** | 11 枚 | 1 桁 | 0.80 → 0.50 秒 | 2 桁 | × 1.1 |
| **T3** | 12 枚 | 1 桁 | 0.80 → 0.50 秒 | 2 桁 | × 1.25 |
| **T4** | 12 枚 | 1 桁 | 0.65 → 0.40 秒 | 2 桁 | × 1.4 |
| **T5** | 13 枚 | 1 桁 | 0.65 → 0.40 秒 | 2 桁 | × 1.6 |
| **T6** | 12 枚 | 1〜2 桁混合 | 0.70 → 0.42 秒 | 3 桁 | × 2.0 |
| **T7** | 14 枚 | 1〜2 桁混合 | 0.70 → 0.42 秒 | 3 桁 | × 2.5 |
| **T8** | 15 枚 | 2 桁 | 0.50 → 0.28 秒 | 3 桁 | × 3.0 |
...
### 4-2. 速度カーブ (v1.4 で線形補間化)

各番号の表示間隔は `start_ms` から `end_ms` まで**線形補間で滑らかに減速**する。

```
interval(i) = lerp(start_ms, end_ms, i / (count - 1))
```

v1.3 までは「ウォームアップ 30% (slow) → 本番 40% (medium) → 追い込み 30% (fast)」の **3 段階離散カーブ** だったが、フェーズ境界で表示間隔が突然短くなりプレイヤーが「途中から急に早くなる」と感じる UX 問題があったため、線形補間に変更。同時に T1〜T3 (slow クラス) のベースラインを初心者向けに緩和 (旧 600/400/300ms → 新 800→500ms)。

| 速度クラス | 適用ティア | start_ms | end_ms |
|---|---|---|---|
| slow | T1〜T3 | 800 | 500 |
| fast | T4〜T5 | 650 | 400 |
| mixed_slow | T6〜T7 | 700 | 420 |
| extreme | T8 | 500 | 280 |
