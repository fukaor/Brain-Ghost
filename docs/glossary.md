...
このドキュメントは、ブレインゴースト プロジェクトのすべてのドキュメント・コードで使用される用語を統一的に定義する。**新しい概念を導入するときは必ずここに追加する**こと（ユビキタス言語の維持）。
...
**定義**: プレイヤーが過去の自分（「いつもの自分」）と対戦する ブレインゴースト の最大の独自要素。
...
**定義**: ブレインゴースト の **第 3 の独自要素**。ゴースト対戦 (FR-02) のデータを、**ユーザ自身の生霊** として視覚化する再利用可能な UI コンポーネント。全画面共通で登場し、精度 / 脳年齢 / ストリークに応じて見た目が変わり、吹き出しで対話式に案内する。
...
**定義**: 日本民俗学における「生きている人間の魂が肉体から離れて現れる存在」。ブレインゴースト では **ユーザ自身の過去自分の人格化** として借用している（悪霊の意味ではなく、ポジティブな分身として再解釈）。

**ブレインゴースト での用法**: ゴースト対戦のデータ対象である「いつもの自分（直近 5 回平均）」を、生霊という概念で視覚的・物語的に統合した。別のマスコットキャラクタを配置するのではなく、ユーザ本人の分身を隣に立たせることで、自己対戦体験を "鏡を見る" 体験に昇華する。
...
| 注意力 (Attention) | 色文字ストループ | — |
| 反射速度 (Reflex) | ゴースト7番勝負 | — |
| 観察力 (Observation) | 数字さがし | — |
| 判断力 (Judgment) | 神経衰弱ライト | 推理、空間認知 |
...
| 分類 | ゲーム | ゴーストUI |
|---|---|---|
| **タイム系** | ゴースト7番勝負 / フラッシュ暗算 / 色文字ストループ | プレイ中にプログレスバー2本（自分 vs いつもの自分） |
| **クリア系** | 順番記憶 / 神経衰弱ライト / 数字さがし | プレイ中はゴースト非表示、結果画面でタイム比較 |
...
**本プロジェクトでの用途**: ブレインゴースト の唯一のランタイム。単一コードベースから Web (HTML5) と Android を同時エクスポート。
...
**定義**: AdMob が提供する広告フォーマットのうち、ブレインゴースト で使用する 3 種類。
...
**継承関係**: 各ミニゲーム（`flash_calc.gd`, `number_search.gd`, `card_match.gd`, `stroop.gd` 等）は `BaseGame` を継承する。
...
**本プロジェクトでの適用**: ブレインゴースト は MVP ではサーバを持たない。データはローカルに保存され、ネットワーク通信は広告配信時のみ。シェアURL も URL パラメータだけで成立しているためサーバ不要。
...
**保存先**: `user://user_config.json` (Android) / `localStorage["brainghost_user_config"]` (Web)
...
| ゲーム | 式 |
|---|---|
| フラッシュ暗算 | `precomputed_score` をパススルー（`max(0, ghost_delta_ms − response_time_ms) + 500) × ティア倍率`） |
| 順番記憶 | `max_level * 150` |
| 色文字ストループ | `precomputed_score` をパススルー（`max(0, 正答×100 − 誤答×50) × ティア倍率`） |
| ゴースト7番勝負 | `(1000 / 中央 5 発平均ms) × 300 + 勝利数 × 50` |
| 神経衰弱ライト | `((ペア数×2/総タップ数)×1000 + 残り時間比率×500) × ティア倍率` |
| 数字さがし | `1500 × (制限時間 − クリアタイム) / 制限時間 × ティア倍率`（未クリア=0） |
